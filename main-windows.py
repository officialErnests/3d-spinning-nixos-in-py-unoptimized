...
objects_pre = [
    ...
    Obj(
        vec3(0,0,0),
        [
            rtri(points[0],points[1],points[3],'1'),
            rtri(points[0],points[2],points[3],'2'),
            rtri(points[2],points[4],points[3],'3'),
            rtri(points[3],points[4],points[5],'4'),
            rtri(points[3],points[4],points[5],'5'),
            rtri(points[6],points[4],points[5],'6'),
            rtri(points[6],points[4],points[7],'7'),
            rtri(points[0],points[7],points[6],'8'),
            rtri(points[0],points[1],points[6],'9')
        ])
]
objects = [
    objects_pre[0].cmov(vec3(0,0,0)),
    objects_pre[0].cmov(vec3(0,-10,0)),
    objects_pre[0].cmov(vec3(10,0,0)),
]
# MAIN LOOP ##
tick = 0
while running:
    ...
    if keyboard.is_pressed('e'): move_dir.y -= 1
    if keyboard.is_pressed('q'): move_dir.y += 1
    move_dir = move_dir.rotXY(-camera.angle.x)
    camera.velocity = ((camera.velocity + move_dir) * 0.5).cind(0.1)
    camera.distance += camera.velocity.len()
    camera.pos += camera.velocity
    # turning
    turn = 0
    if keyboard.is_pressed('left'): turn -= math.pi / 32.0
    if keyboard.is_pressed('right'): turn += math.pi / 32.0
    camera.angle.x += turn
    #render
    clearGrid()
    for i in objects:
    ...
    renderGrid()
    #debug
    print("Yobi alpha 0.3v", tick, camera.pos, camera.angle)
